...
workflow_path = "n8n-workflows/TDF-Regrid-Scraper-V15-WORKFLOW.json"

# Create the fixed workflow based on the current state
workflow = {
...
return {
  json: {
    ...
    success: response.success
        ...
        # NEW CODE NODE instead of convertToFile
        {
            "parameters": {
                "jsCode": """// Convert base64 screenshot to binary for upload
    ...
    data: {
      ...
      fileName: cleanParcelId + '.jpeg'
        ...
        },
        {
            "parameters": {
                "method": "POST",
                "url": "=https://oiiwlzobizftprqspbzt.supabase.co/storage/v1/object/screenshots/{{ $json.clean_parcel_id }}.jpeg",
                "sendHeaders": True,
                "headerParameters": {
                    "parameters": [
                        {"name": "apikey", "value": "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Im9paXdsem9iaXpmdHBycXNwYnp0Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NTUwNDc4MzUsImV4cCI6MjA3MDYyMzgzNX0.idwK7IxweMNK64lOGZlWUFV9pA9zljZmZY65rAiDFzg"},
...
# Save to file
with open("temp/workflow_with_code_node.json", "w") as f:
    json.dump(workflow, f, indent=2)

print("Workflow saved to temp/workflow_with_code_node.json")
