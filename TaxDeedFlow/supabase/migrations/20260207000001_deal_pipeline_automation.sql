-- Migration: 20260207000001_deal_pipeline_automation.sql
-- Description: Deal pipeline automation - create_deal_from_property function,
--              auto-advance rules table, auto-advance trigger, auto-deal-creation trigger
-- Author: Claude Code Agent
-- Date: 2026-02-07
-- Dependencies: deals, pipeline_stages, properties, regrid_data, counties tables
...
-- Rule 1: Lead -> Research when has_regrid_data = true (ENABLED)
INSERT INTO pipeline_auto_advance_rules (organization_id, from_stage_name, to_stage_name, trigger_condition, is_enabled)
VALUES ('default', 'Lead', 'Research', 'has_regrid_data = true', TRUE);

-- Rule 2: Research -> Analysis when visual_validation_status = approved (ENABLED)
INSERT INTO pipeline_auto_advance_rules (organization_id, from_stage_name, to_stage_name, trigger_condition, is_enabled)
VALUES ('default', 'Research', 'Analysis', 'visual_validation_status = approved', TRUE);

-- Rule 3: Analysis -> Due Diligence when title_search_complete (DISABLED)
INSERT INTO pipeline_auto_advance_rules (organization_id, from_stage_name, to_stage_name, trigger_condition, is_enabled)
VALUES ('default', 'Analysis', 'Due Diligence', 'title_search_complete', FALSE);

-- Rule 4: Due Diligence -> Bidding when auction_registered (DISABLED)
INSERT INTO pipeline_auto_advance_rules (organization_id, from_stage_name, to_stage_name, trigger_condition, is_enabled)
VALUES ('default', 'Due Diligence', 'Bidding', 'auction_registered', FALSE);
...
-- Function: Check and auto-advance deals when property data changes
...
DECLARE
    ...
    -- Use a system user UUID for automated actions
    -- We'll use the deal's created_by as the actor
    v_actor_id UUID;
BEGIN
    ...
    LOOP
        ...
        -- Check Rule 1: has_regrid_data changed to true
        IF TG_OP = 'UPDATE'
        ...
        -- Check Rule 2: visual_validation_status changed to 'approved'
        IF TG_OP = 'UPDATE'
        ...
        THEN
            -- Re-fetch deal stage name since it may have been advanced above
            ...
            IF v_rule IS NOT NULL THEN
                ...
                IF v_to_stage_id IS NOT NULL THEN
                    -- Get the current stage to make sure we're not already there
...
-- Function: Auto-create a deal when property's visual_validation_status changes to 'approved'
...
BEGIN
    ...
    THEN
        ...
        IF v_lead_stage_id IS NULL THEN
            -- No Lead stage found, skip silently
            RETURN NEW;
        END IF;

        -- Use a system user as created_by
        -- Get the first admin user from the default org, or the first auth user
        ...
        -- If no org members found, try to get any user from auth.users
        ...
        -- If still no user found, we can't create a deal (created_by is NOT NULL)
        IF v_created_by IS NULL THEN
            RETURN NEW;
        END IF;

        -- Create the deal using create_deal_from_property
        PERFORM create_deal_from_property(
            NEW.id,              -- p_property_id
            v_organization_id,   -- p_organization_id
            v_lead_stage_id,     -- p_stage_id
