...
/**
 * GET /api/v1/counties
 * Public API endpoint for retrieving all counties with property counts and auction data
 * Requires API key authentication via x-api-key header
 */
export async function GET(request: NextRequest) {
  // Validate API key and enforce rate limiting
  const authResult = await validateApiKeyWithRateLimit(request, "/api/v1/counties")
  ...
  try {
    ...
    // Get all counties with basic info
    ...
    if (countiesError) {
      console.error("[API v1 Counties] Database error:", countiesError)
      return NextResponse.json(
    ...
    if (propCountError) {
      console.error("[API v1 Counties] Property count error:", propCountError)
    }
    ...
    if (salesError) {
      console.error("[API v1 Counties] Sales error:", salesError)
    }
    ...
    if (docsError) {
      console.error("[API v1 Counties] Documents error:", docsError)
    }
    ...
    if (progressError) {
      console.error("[API v1 Counties] Progress error:", progressError)
    }
    ...
    const transformedCounties = (counties || []).map((county) => {
      ...
      let nextAuctionIsoDate: string | null = null
      if (nextSale) {
        ...
        nextAuctionIsoDate = nextSale.date.toISOString()
      }
      ...
      return {
        ...
        next_auction: {
          date: nextAuctionIsoDate,
          date_formatted: nextAuctionDate,
          days_until: daysUntilAuction,
          property_count: nextSale?.propertyCount || 0
        },
        documents_count: documentCount,
        last_researched_at: county.last_researched_at,
    ...
    console.error("[API v1 Counties] Server error:", error)
    return NextResponse.json(
