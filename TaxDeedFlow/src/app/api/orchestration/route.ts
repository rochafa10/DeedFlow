...
export async function GET() {
  try {
    ...
    const pipelineMetrics = pipelineMetricsResult.data || []
    const properties = propertiesStatsResult.data || []
    ...
    const transformedAssignments = assignments.map((assignment: any) => ({
      ...
      task: `${formatTaskType(assignment.task_type)} - ${assignment.counties?.county_name || "Unknown"}`,
      ...
      progress: assignment.items_total > 0
        ? Math.round((assignment.items_processed / assignment.items_total) * 100)
        : 0,
      itemsTotal: assignment.items_total || 0,
      itemsProcessed: assignment.items_processed || 0,
      itemsFailed: assignment.items_failed || 0,
    ...
    return NextResponse.json({
      data: {
        ...
        // Recent metrics
        recentMetrics: pipelineMetrics.slice(0, 20).map((m: any) => ({
          stage: m.stage,
          county: m.county_name,
          pendingCount: m.pending_count,
          completedToday: m.completed_today,
          bottleneckScore: m.bottleneck_score,
...
function generateWorkQueue(counties: any[], properties: any[], batchJobs: any[]) {
  const workQueue: any[] = []
  let priorityCounter = 1

  // Count properties needing regrid by county
  const countyPropertyMap = new Map<string, { total: number; needsRegrid: number; needsValidation: number; county: any }>()
  ...
  for (const [countyId, data] of Array.from(countyPropertyMap)) {
    if (data.total > 0) {
      ...
      const activeJob = batchJobs.find(
        (j: any) => j.county_id === countyId && ["pending", "in_progress", "paused"].includes(j.status)
      )
...
// Generate session plan recommendations
function generateSessionPlan(workQueue: any[]) {
...
// Get agent status
function getAgentStatus(assignments: any[], batchJobs: any[]) {
  const agents = [
  ...
  return agents.map((agent) => {
    ...
    const activeAssignment = assignments.find(
      (a: any) => a.task_type === agent.type && a.status === "in_progress"
    )

    // Check for active batch jobs
    const activeJob = batchJobs.find(
      (j: any) => j.job_type === agent.type && ["in_progress", "paused"].includes(j.status)
    )
    ...
    if (activeAssignment) {
      status = "active"
      currentTask = `${activeAssignment.counties?.county_name || "Unknown"} - ${activeAssignment.items_processed}/${activeAssignment.items_total}`
      progress = activeAssignment.items_total > 0
        ? Math.round((activeAssignment.items_processed / activeAssignment.items_total) * 100)
        : 0
    } else if (activeJob) {
      status = activeJob.status === "paused" ? "paused" : "active"
      currentTask = `${activeJob.counties?.county_name || "Unknown"} - ${activeJob.processed_items}/${activeJob.total_items}`
