...
export default function PropertyReportPage() {
  ...
  // State for Regrid re-scrape
  const [isScraping, setIsScraping] = useState(false);

  // Handle Regrid re-scrape via n8n webhook
  const handleRescrape = async () => {
    if (!property || isScraping) return;
    setIsScraping(true);
    try {
      ...
      if (data.success || data.regrid_data) {
        // Refresh the page data to show updated screenshot
        window.location.reload();
      } else {
        console.error("Scrape failed:", data.error);
        alert(data.error || "Failed to scrape Regrid data. Please try again.");
      }
    } catch (error) {
    ...
    <div id="report-container" className="min-h-screen bg-slate-50 dark:bg-slate-950">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
        ...
        <section className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          {/* Regrid Aerial View */}
          <div className="rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-6">
            <h3 className="flex items-center justify-between text-lg font-semibold text-slate-900 dark:text-slate-100 mb-4">
              ...
              <button
                ...
                disabled={isScraping}
                className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded-md bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400 hover:bg-amber-200 dark:hover:bg-amber-900/50 transition-colors disabled:opacity-50"
                title={property?.screenshot_url ? "Re-capture Regrid screenshot" : "Capture Regrid screenshot"}
              >
                {isScraping ? (
                  <>
                  ...
                  <>
                    ...
                    {property?.screenshot_url ? 'Re-scrape' : 'Scrape'}
                  </>
                )}
              </button>
            </h3>
        ...
        <section className="rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            <div className="p-4 rounded-lg bg-slate-50 dark:bg-slate-700/50">
              ...
              <p className="text-2xl font-bold text-slate-900 dark:text-slate-100">
                {elevationData ? `${typeof elevationData.elevation === 'object' && elevationData.elevation !== null ? elevationData.elevation.meters : elevationData.elevation}m` : '358m'}
              </p>
              <p className="text-sm text-slate-500 dark:text-slate-400">
                {elevationData ? `${typeof elevationData.elevation === 'object' && elevationData.elevation !== null ? elevationData.elevation.feet : (elevationData.elevationFeet ?? Math.round((Number(elevationData.elevation) || 0) * 3.28084))}ft` : '1,175ft'} above sea level
              </p>
            </div>
            <div className="p-4 rounded-lg bg-slate-50 dark:bg-slate-700/50">
