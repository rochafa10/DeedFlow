 ...
 *
 * @throws {Error} If the Web Crypto API is not available
 * @returns {string} A 64-character hexadecimal token
 */
export function generateCsrfToken(): string {
  // Ensure crypto API is available (Node.js 18+ / modern browsers)
  if (typeof crypto === "undefined" || !crypto.getRandomValues) {
    throw new Error(
      "Crypto API is not available. Cannot generate secure CSRF token. " +
      "Please ensure you are running Node.js 18+ or a modern browser."
    )
  }
