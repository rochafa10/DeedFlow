#!/usr/bin/env node
/**
 * Regrid Screenshot Capture Script v10
 *
 * FIXES in v10:
 * - REMOVED ALL Escape key presses - Escape was closing the entire page/browser
 * - Skip panel closing entirely - just take screenshot with panel visible
 * - Can iterate to close panel differently in v11 if needed
 *
 * FIXES in v9:
 * - Fixed map click coordinates from (200, 400) to (960, 540)
 *
 * Usage: node regrid-screenshot-v10.js <parcel_id> <county> <state> <property_id>
...
(async () => {
  ...
  try {
    ...
    // Step 8: v10 - NO ESCAPE KEY! Just wait for map to settle
    // Escape was closing the entire page/browser context
    console.error('[Screenshot] Skipping panel close (v10 - no Escape key)...');
    await page.waitForTimeout(2000);

    // Step 9: Take screenshot (with Property Details panel if visible)
