-- Deal Pipeline Automation Schema v2 (Corrected)
...
--
-- Fixes from v1:
--   1. organization_id matched to actual DB type (TEXT, default 'default')
--   2. created_by matched to actual DB type (TEXT, nullable)
--   3. No dangerous DELETE statements (test data already cleaned)
--   4. RAISE NOTICE on all failure paths for debugging
--   5. Added composite index on deals(property_id, status)
--   6. Deterministic user selection: oldest admin of org, then oldest active member
--   7. deal_activities.user_id is TEXT (matches live schema)
--
-- IMPORTANT: The live database uses TEXT for organization_id and created_by,
-- NOT UUID as declared in the schema SQL file. This migration matches the
-- actual live column types.
--
-- Prerequisites:
...
-- ============================================================================
-- CHUNK 1: Drop old objects + Create pipeline_auto_advance_rules table
-- ============================================================================

-- Drop old triggers first (must happen before dropping functions)
...
-- Drop old functions (all possible signatures from v1 attempts)
DROP FUNCTION IF EXISTS auto_create_deal_on_approval();
DROP FUNCTION IF EXISTS check_deal_auto_advance();
DROP FUNCTION IF EXISTS create_deal_from_property(UUID, TEXT, UUID, UUID, TEXT);
DROP FUNCTION IF EXISTS create_deal_from_property(UUID, UUID, UUID, UUID, TEXT);

-- Drop old table if it exists (may have wrong column types from v1)
DROP TABLE IF EXISTS pipeline_auto_advance_rules;

-- Create table with TEXT type for organization_id (matches deals/pipeline_stages)
CREATE TABLE pipeline_auto_advance_rules (
...
-- Insert default rules (organization_id = 'default' to match pipeline_stages)
...
-- ============================================================================
-- CHUNK 2: Create create_deal_from_property() function
-- organization_id is TEXT, created_by is TEXT (matches live DB)
-- ============================================================================
...
BEGIN
    ...
    IF v_existing_deal_id IS NOT NULL THEN
        RAISE NOTICE 'create_deal_from_property: Active deal % already exists for property % in org %',
            v_existing_deal_id, p_property_id, p_organization_id;
    ...
    IF NOT FOUND THEN
        RAISE NOTICE 'create_deal_from_property: Property not found: %', p_property_id;
    ...
    IF v_assessed_value IS NULL THEN
        RAISE NOTICE 'create_deal_from_property: No regrid assessed/market value found for property %, using NULL', p_property_id;
    END IF;
    ...
    -- Create the deal
    -- Note: organization_id=TEXT, created_by=TEXT (nullable), current_stage_id=UUID
    INSERT INTO deals (
        organization_id,
        property_id,
        title,
    ...
    RAISE NOTICE 'create_deal_from_property: Created deal % for property % (%) in org %',
        v_deal_id, p_property_id, v_title, p_organization_id;

    -- Log deal creation activity
    -- deal_activities.user_id is TEXT and nullable
...
COMMENT ON FUNCTION create_deal_from_property IS 'Creates a deal from a property, auto-populating title, description, financial data, and auction date from properties/regrid_data/counties tables. Returns existing deal_id if one already exists. organization_id and created_by are TEXT to match live schema.';


-- ============================================================================
-- CHUNK 3: Create check_deal_auto_advance() + trigger
-- ============================================================================
...
DECLARE
    ...
    v_actor_id TEXT;  -- TEXT to match deals.created_by
BEGIN
    ...
    LOOP
        ...
        THEN
            ...
            IF v_rule IS NOT NULL THEN
                ...
                IF v_to_stage_id IS NOT NULL AND v_to_stage_id != v_deal.current_stage_id THEN
                    RAISE NOTICE 'check_deal_auto_advance: Advancing deal % from "%" to "%" (has_regrid_data rule)',
                        v_deal.deal_id, v_deal.current_stage_name, v_rule.to_stage_name;
                ...
                ELSE
                    RAISE NOTICE 'check_deal_auto_advance: Target stage "%" not found or same as current for deal % in org "%"',
                        v_rule.to_stage_name, v_deal.deal_id, v_deal.organization_id;
                END IF;
            ELSE
                RAISE NOTICE 'check_deal_auto_advance: No enabled has_regrid_data rule found for stage "%" in org "%"',
                    v_deal.current_stage_name, v_deal.organization_id;
            END IF;
        END IF;
        ...
        THEN
            -- Re-fetch deal stage name (may have been advanced by the rule above)
            ...
            IF v_rule IS NOT NULL THEN
                ...
                IF v_to_stage_id IS NOT NULL THEN
                    ...
                    IF v_to_stage_id != v_from_stage_id THEN
                        RAISE NOTICE 'check_deal_auto_advance: Advancing deal % from "%" to "%" (visual_validation rule)',
                            v_deal.deal_id, v_current_stage_name, v_rule.to_stage_name;
                        PERFORM move_deal_to_stage(
                            v_deal.deal_id,
                            v_to_stage_id,
                            v_actor_id,
                    ...
                    ELSE
                        RAISE NOTICE 'check_deal_auto_advance: Target stage same as current for deal %, skipping', v_deal.deal_id;
                    END IF;
                ELSE
                    RAISE NOTICE 'check_deal_auto_advance: Target stage "%" not found for deal % in org "%"',
                        v_rule.to_stage_name, v_deal.deal_id, v_deal.organization_id;
                END IF;
            ELSE
                RAISE NOTICE 'check_deal_auto_advance: No enabled visual_validation rule found for stage "%" in org "%"',
                    v_current_stage_name, v_deal.organization_id;
            END IF;
        END IF;
...
COMMENT ON FUNCTION check_deal_auto_advance IS 'Trigger function that auto-advances deals through pipeline stages when property data changes (has_regrid_data, visual_validation_status). Uses TEXT org_id matching live schema.';
...
-- ============================================================================
-- CHUNK 4: Create auto_create_deal_on_approval() + trigger
-- ============================================================================

CREATE OR REPLACE FUNCTION auto_create_deal_on_approval()
RETURNS TRIGGER AS $$
DECLARE
    v_organization_id TEXT := 'default';  -- TEXT, matches pipeline_stages/deals
    v_lead_stage_id UUID;
    v_created_by TEXT;  -- TEXT, nullable in live schema
    v_existing_deal_id UUID;
BEGIN
    ...
    IF TG_OP = 'UPDATE'
       ...
       AND (OLD.visual_validation_status IS NULL OR OLD.visual_validation_status != 'approved')
    THEN
        -- Check if property already has an active deal (in any org)
        SELECT id INTO v_existing_deal_id
        FROM deals
        ...
        IF v_existing_deal_id IS NOT NULL THEN
            RAISE NOTICE 'auto_create_deal_on_approval: Property % already has active deal %, skipping',
                NEW.id, v_existing_deal_id;
            RETURN NEW;
        END IF;
        ...
        IF v_lead_stage_id IS NULL THEN
            RAISE NOTICE 'auto_create_deal_on_approval: No active "Lead" pipeline stage found for org "%". Cannot auto-create deal for property %',
                v_organization_id, NEW.id;
            RETURN NEW;
        END IF;

        -- Try to find an admin user for the organization
        -- organization_members may not be accessible, so we use a safe approach
        -- First try: get admin from organization_members if the table exists
        BEGIN
            SELECT om.user_id::TEXT INTO v_created_by
        ...
        EXCEPTION WHEN undefined_table THEN
            RAISE NOTICE 'auto_create_deal_on_approval: organization_members table not found, created_by will be NULL';
            v_created_by := NULL;
        END;

        -- Fallback: any active member of the organization
        ...
        -- Last resort fallback: get any user from auth.users (deterministic: oldest)
        IF v_created_by IS NULL THEN
            BEGIN
                SELECT id::TEXT INTO v_created_by
                FROM auth.users
                ORDER BY created_at ASC
                LIMIT 1;
            EXCEPTION WHEN OTHERS THEN
                RAISE NOTICE 'auto_create_deal_on_approval: Could not query auth.users: %', SQLERRM;
                v_created_by := NULL;
            END;
        END IF;

        -- created_by is nullable in live schema, so NULL is acceptable
        IF v_created_by IS NULL THEN
            RAISE NOTICE 'auto_create_deal_on_approval: No user found for created_by. Deal will have NULL created_by for property %', NEW.id;
        END IF;

        RAISE NOTICE 'auto_create_deal_on_approval: Creating deal for property % in org "%" by user %',
            NEW.id, v_organization_id, COALESCE(v_created_by, 'NULL');
...
COMMENT ON FUNCTION auto_create_deal_on_approval IS 'Automatically creates a deal in the Lead stage when a property visual_validation_status changes to approved. Uses TEXT org_id ("default"), deterministic user selection with safe fallbacks. created_by can be NULL if no user found.';
...
-- ============================================================================
-- CHUNK 5: Create composite index on deals(property_id, status)
-- ============================================================================
